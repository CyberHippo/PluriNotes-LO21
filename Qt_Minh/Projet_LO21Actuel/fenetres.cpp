...
//Plus nécessaire si on regroupe tout sous édition
      /*  QAction *actionModifier = new QAction("&Modifier", this);
        actionModifier->setShortcut(QKeySequence("Ctrl+M"));
        connect(actionModifier,SIGNAL(triggered()),this,SLOT(ouverture_modif()));
...
/*void Principale:: ouverture_modif() { // creer une classe pour une fenêtre spéciale d'édition

}
*/

void Principale::creation_docks(){
//1 er Dock
    dock_affichage_notes = new QDockWidget(tr("Notes"), this);
         dock_affichage_notes->setAllowedAreas(Qt::LeftDockWidgetArea |
                                     Qt::RightDockWidgetArea);
        liste_Notes = new QListWidget(dock_affichage_notes);
         dock_affichage_notes->setWidget(liste_Notes);
         addDockWidget(Qt::LeftDockWidgetArea, dock_affichage_notes);

  //2nd Dock:


      //  dock_affichage_arborescence =new QDockWidget(tr("Arborescence"), this);
      //   dock_affichage_arborescence->setAllowedAreas(Qt::LeftDockWidgetArea |
       //                              Qt::RightDockWidgetArea);

       //  dock_affichage_arborescence->setWidget(view);
       //  addDockWidget(Qt::RightDockWidgetArea, dock_affichage_arborescence);*/
}
